...
export function ExamOverview() {
  const { domains, examMeta, examScenarios, hasScenarios, sampleQuestions, certification } = useContent()
    ...
    <div className="animate-fade-in max-w-4xl">
      ...
      <p className="mt-2 max-w-reading text-ink-soft dark:text-stone-300">
        {certification === 'developer' ? (
          <>
            Validates that you can build, integrate, and ship production-grade applications, agents, and workflows using
            Claude at a foundational level — API integration, agents, tools/MCP, evals, security, and model selection.
            {examMeta.examCode && (
              <>
                {' '}
                Exam code <span className="font-mono">{examMeta.examCode}</span>
                {examMeta.itemCount != null && <> · {examMeta.itemCount} items</>}
                {examMeta.timeLimitMinutes != null && <> · {examMeta.timeLimitMinutes} minutes</>}.
              </>
            )}
          </>
        ) : (
      ...
      {hasScenarios && examScenarios.length > 0 && (
        <>
          <h2 className="mb-2 mt-8 text-xl font-bold">The {examScenarios.length} scenarios</h2>
      ...
      {!hasScenarios && (
        <p className="mt-8 max-w-reading text-sm text-ink-soft dark:text-stone-300">{examMeta.scenarioNote}</p>
      )}

      <div className="mt-8 flex flex-wrap gap-3">
        <Link to="/exam/quiz" className="rounded-md bg-accent-600 px-4 py-2 text-sm font-medium text-white hover:bg-accent-700">
          Take the official {sampleCount}-question quiz
        </Link>
